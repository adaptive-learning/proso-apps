...
class ScheduledEmailManager(models.Manager):
    ...
    def send(self, n=100, auth_user=None, auth_password=None):
        emails = list(self.select_related('user').filter(status=ScheduledEmail.STATUS_SCHEDULED, scheduled__gt=datetime.datetime.now() - datetime.timedelta(minutes=130)).order_by('-scheduled')[:n])
