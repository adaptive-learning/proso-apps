        ...
        with closing(connection.cursor()) as cursor_source:
            cursor_source.execute(
                ...
                SELECT
                    ...
                    ip_address
                FROM
                    proso_questions_decoratedanswer
                ''')
            with closing(connection.cursor()) as cursor_dest:
                for row in cursor_source:
                    cursor_dest.execute(
                        ...
                        WHERE id = %s
                        ''', [row[1], row[0]])
        # Deleting field 'DecoratedAnswer.ip_address'
        db.delete_column(u'proso_questions_decoratedanswer', 'ip_address')


    def backwards(self, orm):
        # Adding field 'DecoratedAnswer.ip_address'
        db.add_column(u'proso_questions_decoratedanswer', 'ip_address',
                      self.gf('django.db.models.fields.CharField')(default=None, max_length=39, null=True, blank=True),
                      keep_default=False)
