...
class TestItemManager(test.TestCase):
    ...
    def setUp(self):
        ...
        self._flashcards = dict([((f, f.lang), f) for f in Flashcard.objects.select_related('term', 'context').all()])
        call_command('find_item_types')
        call_command('fill_item_types')

    def test_get_json_objects(self, language='cs'):
        all_objects = {}
        ...
        json_objects = Item.objects.get_json_objects(list(all_objects.keys()), 'cs')
